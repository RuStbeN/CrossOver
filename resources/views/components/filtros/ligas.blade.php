...
<div class="p-6 border-b border-gray-200 dark:border-dark-700 bg-white dark:bg-dark-800" 
    ...
    <form method="GET" action="{{ route('ligas.index') }}" class="flex flex-col lg:flex-row gap-4 mb-4">
        <div class="flex-1 relative">
            ...
            <input 
                ...
                name="search"
                x-model="searchTerm"
                placeholder="Buscar ligas por nombre o descripción..."
                value="{{ $searchTerm }}"
    ...
    <div x-show="showFilters" 
        ...
        <form method="GET" action="{{ route('ligas.index') }}" id="filtrosForm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="ordenar" x-model="selectedOrdenar" class="w-full px-3 py-2 border border-gray-300 dark:border-dark-600 rounded-md bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        ...
                        <option value="categorias_count" {{ $selectedOrdenar == 'categorias_count' ? 'selected' : '' }}>Número de categorías</option>
                        <option value="equipos_count" {{ $selectedOrdenar == 'equipos_count' ? 'selected' : '' }}>Número de equipos</option>
                    </select>
                </div>
            ...
            <div class="flex justify-between items-center mt-4 pt-4 border-t border-gray-200 dark:border-dark-600">
                <div class="flex items-center text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <span>
                        @if($ligasFiltered > 0 || $searchTerm || $selectedEstado || $selectedOrdenar != 'created_at' || $selectedDireccion != 'desc')
                            Mostrando {{ $ligasFiltered }} de {{ $totalLigas }} ligas
                        @else
                            Mostrando {{ $totalLigas }} ligas
                        @endif
                    </span>
                </div>
    ...
    @if($searchTerm || $selectedEstado || $selectedOrdenar != 'created_at' || $selectedDireccion != 'desc')
            ...
            @if($selectedOrdenar != 'created_at')
                @php
                    $ordenamientos = [
                        ...
                        'categorias_count' => 'Número de categorías',
                        'equipos_count' => 'Número de equipos'
                    ];
                @endphp
