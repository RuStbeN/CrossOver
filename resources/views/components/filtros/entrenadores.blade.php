...
<div class="p-6 border-b border-gray-200 dark:border-dark-700 bg-white dark:bg-dark-800" 
    ...
    <form method="GET" action="{{ route('entrenadores.index') }}" class="flex flex-col lg:flex-row gap-4 mb-4">
        <div class="flex-1 relative">
            ...
            <input 
                ...
                name="search"
                x-model="searchTerm"
                placeholder="Buscar entrenadores por nombre, email o cédula..."
                value="{{ $searchTerm }}"
    ...
    <div x-show="showFilters" 
        ...
        <form method="GET" action="{{ route('entrenadores.index') }}" id="filtrosForm">
            ...
            <div class="flex justify-between items-center mt-4 pt-4 border-t border-gray-200 dark:border-dark-600">
                <div class="flex items-center text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <span>
                        @if($entrenadoresFiltered > 0 || $searchTerm || $selectedEstado || $selectedOrdenar != 'created_at' || $selectedDireccion != 'desc')
                            Mostrando {{ $entrenadoresFiltered }} de {{ $totalEntrenadores }} entrenadores
                        @else
                            Mostrando {{ $totalEntrenadores }} entrenadores
                        @endif
                    </span>
                </div>
