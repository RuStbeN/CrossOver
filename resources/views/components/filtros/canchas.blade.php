@props([
    ...
    'selectedTipoSuperficie' => request('tipo_superficie', ''),
    'selectedTechada' => request('techada', ''),
    'selectedIluminacion' => request('iluminacion', ''),
...
<div class="p-6 border-b border-gray-200 dark:border-dark-700 bg-white dark:bg-dark-800" 
     x-data="{ 
        ...
        selectedTipoSuperficie: '{{ $selectedTipoSuperficie }}',
        selectedTechada: '{{ $selectedTechada }}',
        selectedIluminacion: '{{ $selectedIluminacion }}',
        selectedOrdenar: '{{ $selectedOrdenar }}',
        selectedDireccion: '{{ $selectedDireccion }}'
     }">
    
    <!-- Barra de búsqueda principal -->
    <form method="GET" action="{{ route('canchas.index') }}" class="flex flex-col lg:flex-row gap-4 mb-4">
        <div class="flex-1 relative">
            ...
            <input 
                ...
                name="search"
                x-model="searchTerm"
                placeholder="Buscar canchas por nombre o dirección..."
                value="{{ $searchTerm }}"
    ...
    <div x-show="showFilters" 
        ...
        <form method="GET" action="{{ route('canchas.index') }}" id="filtrosForm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <!-- Filtro por Tipo de Superficie -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tipo de superficie</label>
                    <select name="tipo_superficie" x-model="selectedTipoSuperficie" class="w-full px-3 py-2 border border-gray-300 dark:border-dark-600 rounded-md bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        <option value="">Todos los tipos</option>
                        @foreach($tiposSuperficie as $tipo)
                            <option value="{{ $tipo }}" {{ $selectedTipoSuperficie == $tipo ? 'selected' : '' }}>
                                {{ ucfirst($tipo) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <!-- Filtro por Techada -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Techada</label>
                    <select name="techada" x-model="selectedTechada" class="w-full px-3 py-2 border border-gray-300 dark:border-dark-600 rounded-md bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        <option value="">Todas</option>
                        <option value="1" {{ $selectedTechada == '1' ? 'selected' : '' }}>Sí</option>
                        <option value="0" {{ $selectedTechada == '0' ? 'selected' : '' }}>No</option>
                    </select>
                </div>
                
                <!-- Filtro por Iluminación -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Iluminación</label>
                    <select name="iluminacion" x-model="selectedIluminacion" class="w-full px-3 py-2 border border-gray-300 dark:border-dark-600 rounded-md bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        <option value="">Todas</option>
                        <option value="1" {{ $selectedIluminacion == '1' ? 'selected' : '' }}>Sí</option>
                        <option value="0" {{ $selectedIluminacion == '0' ? 'selected' : '' }}>No</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="ordenar" x-model="selectedOrdenar" class="w-full px-3 py-2 border border-gray-300 dark:border-dark-600 rounded-md bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        ...
                        <option value="capacidad" {{ $selectedOrdenar == 'capacidad' ? 'selected' : '' }}>Capacidad</option>
                        <option value="tarifa_por_hora" {{ $selectedOrdenar == 'tarifa_por_hora' ? 'selected' : '' }}>Tarifa por hora</option>
            ...
            <div class="flex justify-between items-center mt-4 pt-4 border-t border-gray-200 dark:border-dark-600">
                <div class="flex items-center text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <span>
                        @if($canchasFiltered > 0 || $searchTerm || $selectedEstado || $selectedTipoSuperficie || $selectedTechada || $selectedIluminacion || $selectedOrdenar != 'created_at' || $selectedDireccion != 'desc')
                            Mostrando {{ $canchasFiltered }} de {{ $totalCanchas }} canchas
                        @else
                            Mostrando {{ $totalCanchas }} canchas
                        @endif
                    </span>
                </div>
    ...
    @if($searchTerm || $selectedEstado || $selectedTipoSuperficie || $selectedTechada || $selectedIluminacion || $selectedOrdenar != 'created_at' || $selectedDireccion != 'desc')
        <div class="mt-4 flex flex-wrap gap-2">
            ...
            @if($selectedTipoSuperficie)
                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                    Superficie: {{ ucfirst($selectedTipoSuperficie) }}
                    <a href="{{ request()->fullUrlWithQuery(['tipo_superficie' => null]) }}" class="ml-1 text-blue-600 hover:text-blue-500">
                        <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
            ...
            @if($selectedTechada !== '')
                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200">
                    Techada: {{ $selectedTechada == '1' ? 'Sí' : 'No' }}
                    <a href="{{ request()->fullUrlWithQuery(['techada' => null]) }}" class="ml-1 text-purple-600 hover:text-purple-500">
                        <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
            ...
            @if($selectedIluminacion !== '')
                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200">
                    Iluminación: {{ $selectedIluminacion == '1' ? 'Sí' : 'No' }}
                    <a href="{{ request()->fullUrlWithQuery(['iluminacion' => null]) }}" class="ml-1 text-yellow-600 hover:text-yellow-500">
                        <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
            ...
            @if($selectedOrdenar != 'created_at')
                ...
                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-indigo-100 dark:bg-indigo-900 text-indigo-800 dark:text-indigo-200">
                    ...
                    <a href="{{ request()->fullUrlWithQuery(['ordenar' => null]) }}" class="ml-1 text-indigo-600 hover:text-indigo-500">
                        <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
            ...
            @if($selectedDireccion != 'desc')
                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-pink-100 dark:bg-pink-900 text-pink-800 dark:text-pink-200">
                    ...
                    <a href="{{ request()->fullUrlWithQuery(['direccion' => null]) }}" class="ml-1 text-pink-600 hover:text-pink-500">
                        <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
