@props([
    ...
    'selectedEntrenador' => request('entrenador_id', ''),
    'selectedOrdenar' => request('ordenar', 'nombre'),
    'selectedDireccion' => request('direccion', 'asc'),
    'totalEquipos' => 0,
    'equiposFiltered' => 0,
    'categorias' => [],
    'ligas' => [],
...
<div class="p-6 border-b border-gray-200 dark:border-dark-700 bg-white dark:bg-dark-800" 
    ...
    <form method="GET" action="{{ route('equipos.index') }}" class="flex flex-col lg:flex-row gap-4 mb-4">
    ...
    <div x-show="showFilters" 
        ...
        <form method="GET" action="{{ route('equipos.index') }}" id="filtrosForm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Entrenador</label>
                    <select name="entrenador_id" x-model="selectedEntrenador" class="w-full px-3 py-2 border border-gray-300 dark:border-dark-600 rounded-md bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        <option value="">Todos los entrenadores</option>
                        @foreach($entrenadores as $entrenador)
                            <option value="{{ $entrenador->id }}" {{ $selectedEntrenador == $entrenador->id ? 'selected' : '' }}>
                                {{ $entrenador->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="ordenar" x-model="selectedOrdenar" class="w-full px-3 py-2 border border-gray-300 dark:border-dark-600 rounded-md bg-white dark:bg-dark-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        ...
                        <option value="fecha_fundacion" {{ $selectedOrdenar == 'fecha_fundacion' ? 'selected' : '' }}>Fecha de fundación</option>
                    </select>
                </div>
            ...
            <div class="flex justify-between items-center mt-4 pt-4 border-t border-gray-200 dark:border-dark-600">
                <div class="flex items-center text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <span>
                        @if($equiposFiltered > 0 || $searchTerm || $selectedEstado || $selectedCategoria || $selectedLiga || $selectedEntrenador || $selectedOrdenar != 'nombre' || $selectedDireccion != 'asc')
                            Mostrando {{ $equiposFiltered }} de {{ $totalEquipos }} equipos
                        @else
                            Mostrando {{ $totalEquipos }} equipos
                        @endif
                    </span>
                </div>
    ...
    @if($searchTerm || $selectedEstado || $selectedCategoria || $selectedLiga || $selectedEntrenador || $selectedOrdenar != 'nombre' || $selectedDireccion != 'asc')
        <div class="mt-4 flex flex-wrap gap-2">
            ...
            @if($selectedCategoria)
                @php
                    $categoria = $categorias->firstWhere('id', $selectedCategoria);
                @endphp
                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                    Categoría: {{ $categoria->nombre ?? '' }}
                    <a href="{{ request()->fullUrlWithQuery(['categoria_id' => null]) }}" class="ml-1 text-blue-600 hover:text-blue-500">
                        <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
            ...
            @if($selectedLiga)
                @php
                    $liga = $ligas->firstWhere('id', $selectedLiga);
                @endphp
                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200">
                    Liga: {{ $liga->nombre ?? '' }}
                    <a href="{{ request()->fullUrlWithQuery(['liga_id' => null]) }}" class="ml-1 text-purple-600 hover:text-purple-500">
                        <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
            ...
            @if($selectedEntrenador)
                @php
                    $entrenador = $entrenadores->firstWhere('id', $selectedEntrenador);
                @endphp
                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200">
                    ...
                    <a href="{{ request()->fullUrlWithQuery(['entrenador_id' => null]) }}" class="ml-1 text-yellow-600 hover:text-yellow-500">
                        <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
