<div class="px-6 py-4 border-b border-gray-400 dark:border-dark-700 bg-gray-100 dark:bg-dark-900 flex items-center justify-between">
    <h2 class="text-2xl font-bold text-black dark:text-primary-400">Temporadas Registradas</h2>
    <button
        @click="showForm = !showForm; if (!showForm) resetForm()"
        class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-md font-medium transition"
    >
        <span x-text="showForm ? 'Ocultar Formulario' : 'Registrar Nueva Temporada'"></span>
    </button>
</div>
<!-- Incluir el componente de filtros -->
<x-filtros.temporadas 
    :total-temporadas="$totalTemporadas"
    :temporadas-filtered="$temporadas->count()"
/>

<div class="p-6">
    @if($temporadas->isEmpty())
        <div class="text-center py-8 text-gray-400">
            <svg class="mx-auto h-12 w-12 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z"></path>
            </svg>
            <h3 class="mt-2 text-sm font-medium dark:text-gray-300 text-dark-800">No hay temporadas registradas</h3>
            <p class="mt-1 text-sm dark:text-gray-300 text-dark-700">Comienza registrando una nueva temporada.</p>
        </div>
    @else
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-stretch">
            @foreach($temporadas as $temporada)
            <div class="bg-white dark:bg-dark-700 rounded-lg border border-gray-300 dark:border-dark-600 overflow-hidden hover:border-primary-500 transition-colors flex flex-col h-full">
                <div class="p-4 flex-1">
                    <div class="flex items-center justify-between">
                        <h3 class="text-lg font-bold text-gray-800 dark:text-white">
                            {{ $temporada->nombre }}
                        </h3>
                        <span class="px-2 py-1 text-xs font-semibold {{ $temporada->activo ? 'bg-green-500' : 'bg-red-500' }} text-white rounded-full">
                            {{ $temporada->activo ? 'Activa' : 'Inactiva' }}
                        </span>
                    </div>
                    
                    @if($temporada->descripcion)
                        <div class="mt-2 text-sm text-gray-700 dark:text-gray-300">
                            <div class="flex items-start">
                                <svg class="w-4 h-4 mr-1 text-primary-400 mt-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                                <p class="line-clamp-3">{{ $temporada->descripcion }}</p>
                            </div>
                        </div>
                    @endif

                    <div class="mt-3 space-y-2 text-sm text-gray-700 dark:text-gray-300">
                        <div class="flex items-center">
                            <svg class="w-4 h-4 mr-1 text-primary-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                            </svg>
                            <span class="font-medium">Fechas: </span>
                            {{ $temporada->fecha_inicio->format('d/m/Y') }} - {{ $temporada->fecha_fin->format('d/m/Y') }}
                        </div>
                        
                        <div class="flex items-center">
                            <svg class="w-4 h-4 mr-1 text-primary-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                            <span class="font-medium">Horario: </span>
                            {{ \Carbon\Carbon::parse($temporada->horario_inicio)->format('h:i A') }} - {{ \Carbon\Carbon::parse($temporada->horario_fin)->format('h:i A') }}
                        </div>

                        <div class="flex flex-wrap gap-1">
                            @foreach($temporada->diasHabiles->sortBy('dia_semana') as $dia)
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium 
                                    {{ $dia->esLaboral() 
                                        ? 'bg-blue-100 text-blue-800 dark:bg-blue-800/30 dark:text-blue-300' 
                                        : 'bg-green-100 text-green-800 dark:bg-green-800/30 dark:text-green-300' }}">
                                    {{ $dia->dia_semana_corto }}
                                </span>
                            @endforeach
                        </div>
                        
                        <div class="flex items-center">
                            <svg class="w-4 h-4 mr-1 text-primary-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                            </svg>
                            <span class="font-medium">Creada: </span>
                            {{ $temporada->created_at->format('d/m/Y') }}
                        </div>
                    </div>
                </div>
                <div class="bg-gray-100 dark:bg-dark-900 px-4 py-3 flex justify-end space-x-2 border-t border-gray-300 dark:border-dark-700">
                    <button 
                        @click="editTemporada({{ $temporada->toJson() }})"
                        class="text-sm text-primary-600 dark:text-primary-400 hover:text-primary-500 dark:hover:text-primary-300 transition-colors"
                    >
                        Editar
                    </button>
                    <form action="{{ route('temporadas.destroy', $temporada->id) }}" method="POST" class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="text-sm text-red-600 dark:text-red-400 hover:text-red-500 dark:hover:text-red-300 transition-colors"
                            onclick="return confirm('¿Estás seguro de que deseas eliminar esta temporada?')">
                            Eliminar
                        </button>
                    </form>
                </div>
            </div>
            @endforeach
        </div>
        @if($temporadas->hasPages())
            <div class="mt-6 flex justify-between items-center">
                <!-- Botón Anterior -->
                @if($temporadas->onFirstPage())
                    <span class="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400 rounded cursor-not-allowed">Anterior</span>
                @else
                    <a href="{{ $temporadas->previousPageUrl() }}" class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-md font-medium transition">Anterior</a>
                @endif

                <!-- Información de página -->
                <span class="text-gray-700 dark:text-gray-300">
                    Página {{ $temporadas->currentPage() }} de {{ $temporadas->lastPage() }}
                </span>

                <!-- Botón Siguiente -->
                @if($temporadas->hasMorePages())
                    <a href="{{ $temporadas->nextPageUrl() }}" class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-md font-medium transition">Siguiente</a>
                @else
                    <span class="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400 rounded cursor-not-allowed">Siguiente</span>
                @endif
            </div>
        @endif
    @endif
</div>